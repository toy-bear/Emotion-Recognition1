...
# 模型位置
emotion_model_path = 'models/_mini_XCEPTION.102-0.66.hdf5'  #模型的位置
emotion_model = load_model(emotion_model_path, compile=False)  # 载入模型
input_shape = emotion_model.input_shape[1:]  # 模型的输入大小

# 载入数据集
faces, emotions = load_fer2013()  #导入数据集
faces = preprocess_input(faces)
num_samples, num_classes = emotions.shape

# 划分的训练、测试集，train_X,test_X,train_y,test_y = train_test_split(train_data,train_target,test_size=0.3,random_state=5)
#train_data待划分数据集，train_target待划分数据集的划分结果（标签），测试数据集所占的比例
